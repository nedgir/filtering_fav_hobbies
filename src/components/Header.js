...
function Header() {
  ...
  const [hobbyfav,setHobbyfav] = useState([]);
...
//   }
  function hobbyhandler(event){
    if(enterhobby.current.value.trim().length===0){
      setIsValid(false);
    }
    else
    {
...
setHobby((prev)=>[...prev,hb]);
  ...
  function addFav(data){
    ...
    console.log(hobby[index]);
    setHobbyfav((prev)=>[...prev,hobby[index]]);
  }
  return (
    <div>
      ...
      <div class="tab-content" id="pills-tabContent">
        ...
        >
          ...
          <input ref={img} type="text" className="form-control"/>
